...
def probes_paises():
    ...
    for (pais, destino), probes in probes_por_pais_destino.items():
        ...
        latencias = [probe['latencia'] for probe in probes_ordenadas]
        
        label = f'{pais} - {destino}'
        plt.plot(timestamps, latencias, label=label)

    plt.title('Latência ao longo do tempo por país e destino')
    ...
    plt.savefig('comparacao_latencia_probes_pais.png')
...
def probes_continentes():
    ...
    plt.title('Latência ao longo do tempo por continente e destino')
    plt.xlabel('Timestamp')
    plt.ylabel('Latência (ms)')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True)
