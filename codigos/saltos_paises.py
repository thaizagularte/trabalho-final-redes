...
for continente in continentes_unicos:
    ...
    for pais in paises:
        for destino in df_continente['destino'].unique():
            ...
            plt.plot(df_pais_destino['timestamp'], df_pais_destino['quantidade_saltos'], marker='o', label=f'{pais} -> {destino}')

    # Configurações do gráfico
    plt.title(f'Quant. de saltos por país e destino no continente {continente}')
    ...
    plt.savefig(f'saltos_paises_{continente}.png')
