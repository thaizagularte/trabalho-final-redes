...
for continente in continentes_unicos:
    ...
    for pais in paises:
        for destino in df_continente['destino'].unique():
            ...
            plt.plot(df_pais_destino['latencia'], df_pais_destino['quantidade_saltos'], marker='o', label=f'{pais} -> {destino}')

    # Configurações do gráfico
    plt.title(f'Saltos por latência por país e destino no continente {continente}')
    ...
    plt.savefig(f'saltos_latencia_paises_{continente}.png')
